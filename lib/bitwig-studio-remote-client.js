...
module.exports = class Client extends Remote {
   ...
   * @return {Promise}
   */
  async progressBar(numerator, denominator) {
    if (denominator <= 0) {
      return;
    }
    if (this.connected && !this.shutdown) {
      ...
      // log(`in progress: [${bar(numerator)}] (${numerator}/${denominator})`);
      await this.msg(`in progress: [${bar(numerator)}] (${numerator}/${denominator})`);
    }
  }
};
