  ...
  const bws = new BitwigClient(opts.url, {
    ...
    timeout = msec =>
      new Promise((resolve, reject) => {
        timer = setTimeout(() => {
          bws.close()
          reject(new Error(`Connection timeout in ${msec}msec.`))
        }, msec)
      })
  await Promise.race([timeout(opts.timeout), bws.connect(-1)])
  clearTimeout(timer)
...
async function loadClip(clipFile) {
  ...
  remote.notify(
    ...
    [0, Local.isWSL() ? Local.wsl2winPath(clipFile) : clipFile]
  )
  await remote
    .event('cursorTrack.clipLauncherSlotBank.getItemAt.hasContent')
    .atSlot(0)
  ...
  try {
    if (local && !local.closed && remote && remote.isOpen) {
      try {
        ...
        await wait(200)
        remote.close()
        await remote.event('close').occur().within(3).sec().asPromised()
      } finally {
        remote.close()
      }
    } else if (remote) {
