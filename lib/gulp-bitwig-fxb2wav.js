...
const fxb2wav = (file, opts, done) => {
  try {
    _fxb2wav(file, opts, done);
  } catch (err) {
...
const _fxb2wav = async (file, opts, done) => {
  ...
  if (file.data.pluginId !== bitwig._pluginId) {
    await bitwig.loadVST2Device(pluginId);
