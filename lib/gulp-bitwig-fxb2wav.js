    ...
    timeout = msec =>
      new Promise((resolve, reject) => {
        timer = setTimeout(() => {
          ...
          reject(
            new Error(
              `Colud not connect to Bitwig Studio within ${opts.timeout} msec.`
            )
          )
        }, msec)
      })
  ...
  await Promise.all([
    (async () => {
      const result = await remote
        ...
        .sec()
        .asPromised()
      clipName = result.params[1]
      log.debug('bounceClip() clip name:', clipName)
    })(),
    remote
