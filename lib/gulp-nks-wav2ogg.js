...
const optionsDescriptions = {
  ...
  silence: 'threshold level for removing silence from end',
...
async function _wav2ogg(file, opts) {
  const fmt = await wavUtil.readFormat(file.path),
        duration = fmt.dataSize / fmt.byteRate;  // duration seconds of wav
  file.contents = await _ffmpeg(file.path, duration, opts);
  // .wav -> .nksf.ogg
  if (opts.dotPreviews) {
 ...
 */
function _ffmpeg(file, duration, opts) {
  const fadeDuration = duration < opts.fadeout ? duration : opts.fadeout,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    // silence. Default value is 0.02. Allowed range is from 0 to 10.
      .audioFilter(`silenceremove=stop_periods=1:stop_duration=0.02:stop_threshold=${opts.silence}`)
