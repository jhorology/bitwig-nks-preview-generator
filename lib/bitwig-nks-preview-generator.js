const fs = require('fs'),
      ...
      log = logger('bitwig-nks-preview-generator');
 ...
 * @return {stream.Readable} - readable object stream
 */
function nksf2ogg(dir, options, callback = function(){}) {
  ...
  const files = glob.sync(globPattern),
        ...
        files.map(f => fs.existsSync(path.join(path.dirname(f), '.previews', path.basename(f) + '.ogg')) ? 0 : 1)
        .reduce((sum, v) => sum + v);
