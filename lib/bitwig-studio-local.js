...
module.exports = class BitwigStudio {
    ...
    if (!this._defaultExecuteFile) {
      this._defaultExecuteFile = (() => {
        switch (process.platform) {
          case 'win32':
            return this._searchNewestBitwigStudio(
              path.join(process.env.PROGRAMFILES, 'Bitwig Studio')
            )
          case 'darwin':
          ...
          case 'linux':
            if (isWSL()) {
              return this._searchNewestBitwigStudio(
                path.join(wslpath(winenv('PROGRAMFILES')), 'Bitwig Studio')
              )
            } else {
  ...
  static _searchNewestBitwigStudio(folderPath) {
    const subFolders = fs
      .readdirSync(folderPath)
      .filter(
        file =>
          fs.statSync(path.join(folderPath, file)).isDirectory() &&
          fs.existsSync(path.join(folderPath, file, 'Bitwig Studio.exe'))
      )
      .sort()
    ...
    return path.join(folderPath, 'Bitwig Studio.exe')
  }
