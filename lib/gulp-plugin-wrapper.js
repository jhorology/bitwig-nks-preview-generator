...
class PluginProcessingError extends PluginError {
  ...
  constructor(pluginName = '(unnamed)', causedBy, file, stop, options) {
    super(
      pluginName,
      `Failed processing file: "${file.basename}"`,
      Object.assign(
        {
          showStack: true,
          showProperties: true
...
class PluginDiscardFile extends PluginError {
  ...
  constructor(pluginName, reason, file, options) {
    super(
      pluginName,
      `Discard processing file: "${file.basename}" reason: ${reason}`,
      Object.assign(
        {
          showStack: false,
...
function plugin(pluginName, processor, options, throughOpts) {
  const stream = through.obj(function (file, enc, cb) {
    ...
    const onComplete = discardReason => {
      if (discardReason) {
        const wrappedError = new PluginDiscardFile(
          pluginName,
          discardReason,
          file
        )
  ...
  stream.on('data', file => {
    log.info(
      chalk.greenBright.bold(`[${pluginName}] processing complete.`),
      file.relative
    )
  })
  return stream
